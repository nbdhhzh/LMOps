#!/bin/bash
# PR-OPCD: On-policy consolidate step
# Teacher (with GLM-5 hint) guides student (plain prompt) via reverse KL
...
EXP_NAME="${EXP_NAME:-pr-opcd-$(basename $MODEL_PATH)}"
MAX_RESPONSE_LENGTH="${MAX_RESPONSE_LENGTH:-8192}"
KL_LOSS_TYPE="${KL_LOSS_TYPE:-full}"
ACTOR_LR="${ACTOR_LR:-5e-6}"

MAX_PROMPT_LENGTH=$((MAX_RESPONSE_LENGTH + 4096))  # larger for hinted teacher prompts
PPO_MAX_TOKEN_LEN=$((MAX_PROMPT_LENGTH + MAX_RESPONSE_LENGTH))

if [ -z "$EXP_PATH" ]; then
    echo "Usage: $0 MODEL_PATH EXPERIENCE_PATH"
    echo "  EXPERIENCE_PATH: output from pr_extract_1gpu.sh"
    exit 1
fi
