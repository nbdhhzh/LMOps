#!/bin/bash
# PR-CD Step 2: Off-policy training (Forward KL / context distillation)
# Student trains on teacher's hinted responses with KL loss
...
EXP_NAME="${EXP_NAME:-pr-cd-offp-$(basename $MODEL_PATH)}"
...
if [ -z "$OFF_POLICY_DIR" ]; then
    echo "Usage: $0 MODEL_PATH OFF_POLICY_DATA_DIR"
    echo "  OFF_POLICY_DATA_DIR: output from pr_generate_offp_1gpu.sh"
    exit 1
fi
